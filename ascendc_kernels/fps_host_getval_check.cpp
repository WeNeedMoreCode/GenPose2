/**
 * Host wrapper for GetValue on GM check — with SyncAll.
 * Writes known values to input_buf (H2D), launches kernel, reads debug output.
 * Now includes sync buffer allocation for SyncAll.
 */
#include "acl/acl.h"
...
constexpr uint32_t SYNC_SIZE = NUM_CORES * 8;                // 64 int32_t (SyncAll needs 8 per core)
    ...
    float *scratch_host, int scratch_host_size)
{
    ...
    fprintf(stderr, "[getval] Step 5: malloc sync buf (%u int32_t)\n", SYNC_SIZE);
    if (g_sync_buf == nullptr) {
        aclError ret = aclrtMalloc(&g_sync_buf, SYNC_SIZE * sizeof(int32_t),
            ACL_MEM_MALLOC_HUGE_FIRST);
        fprintf(stderr, "[getval] aclrtMalloc(sync) ret = %d\n", (int)ret);
        if (ret != ACL_SUCCESS) return -5;
    }

    // Write known values to input: core 0 → 10.0, core 1 → 20.0, ..., core 7 → 80.0
    fprintf(stderr, "[getval] Step 6: write known values to input GM\n");
    ...
    fprintf(stderr, "[getval] Step 7: memset debug/scratch/sync to known pattern\n");
    ...
    aclrtMemset(g_sync_buf, SYNC_SIZE * sizeof(int32_t), 0x00, SYNC_SIZE * sizeof(int32_t));

    fprintf(stderr, "[getval] Step 8: launch kernel (blockDim=%u)\n", NUM_CORES);
    fprintf(stderr, "[getval]   input=%p debug=%p scratch=%p sync=%p\n",
        g_input_buf, g_debug_buf, g_scratch_buf, g_sync_buf);
    uint32_t launch_ret = aclrtlaunch_fps_custom_getval_check(NUM_CORES, g_stream,
        g_input_buf, nullptr, nullptr, g_debug_buf, g_scratch_buf, g_sync_buf);
    fprintf(stderr, "[getval] aclrtlaunch ret = %u\n", launch_ret);

    fprintf(stderr, "[getval] Step 9: sync stream\n");
    ...
    if (scratch_host != nullptr && scratch_host_size >= (int)INPUT_SIZE) {
        fprintf(stderr, "[getval] Step 10: read scratch GM D2H (%u floats)\n", INPUT_SIZE);
    ...
    if (debug_host != nullptr && debug_host_size >= (int)DEBUG_SIZE) {
        fprintf(stderr, "[getval] Step 11: memcpy debug D2H\n");
