/**
 * Host wrapper for minimal multi-core Duplicate+DataCopy test.
 * Kernel signature matches v3 debug (5 params) for binary compatibility.
 */
#include "acl/acl.h"
#include "aclrtlaunch_fps_custom_minimal_test.h"
#include <cstdint>

constexpr uint32_t NUM_CORES = 8;
constexpr uint32_t CHUNK = 1024 / NUM_CORES;
constexpr uint32_t PER_CORE = 8;
constexpr uint32_t DEBUG_SIZE = NUM_CORES * PER_CORE;
constexpr uint32_t SCRATCH_PER_CORE = 64;

static aclrtStream g_minimal_stream = nullptr;
static void *g_minimal_debug_buf = nullptr;
static void *g_minimal_results_buf = nullptr;
static void *g_minimal_scratch_buf = nullptr;

extern "C" int fps_run_minimal_test(float *debug_host, int debug_host_size)
{
    if (g_minimal_stream == nullptr) {
        aclrtCreateStream(&g_minimal_stream);
    }
    if (g_minimal_debug_buf == nullptr) {
        aclrtMalloc(&g_minimal_debug_buf, DEBUG_SIZE * sizeof(float),
            ACL_MEM_MALLOC_HUGE_FIRST);
    }
    if (g_minimal_results_buf == nullptr) {
        aclrtMalloc(&g_minimal_results_buf, NUM_CORES * CHUNK * sizeof(float),
            ACL_MEM_MALLOC_HUGE_FIRST);
    }
    if (g_minimal_scratch_buf == nullptr) {
        aclrtMalloc(&g_minimal_scratch_buf, NUM_CORES * SCRATCH_PER_CORE * sizeof(float),
            ACL_MEM_MALLOC_HUGE_FIRST);
    }

    aclrtlaunch_fps_custom_minimal_test(NUM_CORES, g_minimal_stream,
        nullptr, nullptr, g_minimal_results_buf, g_minimal_debug_buf, g_minimal_scratch_buf);
    aclrtSynchronizeStream(g_minimal_stream);

    if (debug_host != nullptr && debug_host_size >= (int)DEBUG_SIZE) {
        aclrtMemcpy(debug_host, DEBUG_SIZE * sizeof(float),
            g_minimal_debug_buf, DEBUG_SIZE * sizeof(float),
            ACL_MEMCPY_DEVICE_TO_HOST);
    }

    return 0;
}
