...
def run_one(lib, num_cores, xyz, new_xyz, radius, nsample, label=""):
    B, N, _ = xyz.shape
    M = new_xyz.shape[1]
    radius2 = radius * radius

    print(f"\n{'='*70}")
    print(f"{label} B={B}, N={N}, M={M}, nsample={nsample}, radius={radius}")
    print(f"{'='*70}")
    ...
    # Find and show first divergent query
    if max_diff > 0:
        for b in range(B):
            for m in range(M):
                qdiff = diff[b, m]
                if qdiff.max() > 0:
                    print(f"\n  First divergent query: b={b}, m={m}")
                    print(f"    pt_out  = {pt_out[b, m, :min(16, nsample)].tolist()}")
                    print(f"    ac_out  = {idx_ac[b, m, :min(16, nsample)].tolist()}")
                    return
...
def run_debug():
    ...
    # Try loading saved real data first
    ptx_path = os.path.join(SCRIPT_DIR, 'bq_debug_input.pt')
    if os.path.exists(ptx_path):
        print(f"Loading saved input from {ptx_path}")
        data = torch.load(ptx_path, map_location='npu:0', weights_only=True)
        xyz = data['xyz'].to('npu:0').float()
        new_xyz = data['new_xyz'].to('npu:0').float()
        radius = float(data['radius'])
        nsample = int(data['nsample'])
        run_one(lib, num_cores, xyz, new_xyz, radius, nsample, label="[real data]")
    else:
        print(f"No bq_debug_input.pt found, using random data")
        SHAPES = [
            (1, 1024, 512, 32, 0.02),
            (1, 512, 256, 32, 0.04),
            (1, 256, 128, 32, 0.08),
        ...
        for B, N, M, nsample, radius in SHAPES:
            ...
            run_one(lib, num_cores, xyz, new_xyz, radius, nsample, label="[random]")
